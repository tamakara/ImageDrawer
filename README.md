...
*   **沉浸式详情页 (Detail View)**:
    *   点击图片弹出模态窗口（或独立路由页面）。
    *   **信息查看**: 显示分辨率、文件大小、格式、创建时间、哈希值等元数据。
    *   **编辑功能**: 支持修改图片名称、手动添加/删除/修改标签。
    *   **交互操作**: 下载原图、删除图片。
    *   **移动端优化**: 支持双指缩放查看细节，左右滑动切换上一张/下一张。
...
*   **存储策略 (Storage Strategy)**:
    *   **图片文件**: 直接存储在本地文件系统 (`/data/images/`)。
        *   *理由*: 文件系统读写大文件性能优于数据库 Blob，且方便用户直接通过操作系统访问文件。
